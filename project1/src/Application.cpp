...
Application::Application(const std::string &projectName): projectName(projectName) {}
...
void Application::pickPhysicalDevice() {
    uint32_t deviceCount = 0; // #(graphic card)
...
bool Application::isDeviceSuitable(VkPhysicalDevice device) {
    ...
    if (isSuitable) {
        ...
        if (isSuitable) {
            ...
            isSuitable = !swapChainSupport.formats.empty() && !swapChainSupport.presentModes.empty();
        }
    }
...
void Application::recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
    ...
    // NOTE: Just to set up the project and animate background color.
    // In fact, it can be done by using glfw library.
    float time = glfwGetTime();
    float red = (sin(time) + 1.0f) / 2.0f,
            green = (cos(time) + 1.0f) / 2.0f,
            blue = (sin(time+1.0f) + 1.0f) / 2.0f;
